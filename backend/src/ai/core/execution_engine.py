...
class ExecutionEngine(StepEngine):
    """Legacy full-run plan-walker. Adds ``execute_run`` on top of the
    ``StepEngine`` step surface. ``execute_run`` is the C4 deletion target; the
    loop is the supported run entry point.

    Child entities still run via this engine's ``execute_run`` when
    ``async_child_dispatch`` is OFF (the inline ``_run_child_full`` callback);
    that fallback is removed in C4 PR-6.
    """
