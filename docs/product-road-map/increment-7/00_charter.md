...
> ⚠️ **2026-07-30 — THE BUILT FRONTEND WAS REJECTED ON DESIGN.** All eight workstreams shipped and merged; at owner review the app was rejected on look and feel. `vihara/` is preserved as `vihara-review-rejected/`; `backend/src/ai/genui/` and every D5 contract stand. The redesign runs under its own charter: **[redesign/00_redesign_charter.md](./redesign/00_redesign_charter.md)** — seven findings, four locked decisions, five rounds. Read it before anything below.
>
...
**8. Portrait direction is written now; rasters land before G1.** ✅ **CLOSED 2026-07-30 — and the stated blocker was not real.** The art bible carries the portrait direction and HTML/SVG style specimens in the brand tokens; raster generation was recorded here as waiting on "working ADC (the same blocker voice sits behind) or the owner's own image tool".

> **The blocker was misdiagnosed.** The *user* ADC in `~/.config/gcloud` is expired and cannot refresh non-interactively — but this VM has an **attached service account** (`hirebuddha-vertex-ai`, cloud-platform scope) on the metadata server, which is the path `backend/.env` already documents and the backend already uses. No login was ever required. Twelve portraits were drawn on `imagen-4.0-generate-001` and traced to SVG dot lattices by [redesign/../scripts](../../../vihara/scripts/portraits.py) — see art bible **§7.2a**. Worth remembering as a pattern: *a blocker inherited from another workstream is worth re-testing rather than re-quoting.*
...
| Date | Change |
|---|---|
| 2026-07-30 | v1.4 — **charter decision 8 closed, and its stated blocker was never real.** Portrait rasters were recorded as waiting on working ADC; the VM's attached service account needed no login at all. Twelve generated and traced (art bible §7.2a). Resume the redesign at [redesign/03_resume.md](./redesign/03_resume.md). |
| 2026-07-30 | v1.3 — **the built frontend was rejected on design at owner review.** The increment's backend half stands; the tenant app is redesigned from the visual layer up under [redesign/00_redesign_charter.md](./redesign/00_redesign_charter.md). Four decisions locked there: visual redesign **plus** an IA rebalance promoting the nine working surfaces out of L9-fallback status; the legacy `AnimatedBackground.tsx` ported **verbatim** with a brand re-key offered beside it; `vihara-review-rejected/` kept as a **parts bin** with a named salvage list; and review moved to a **pixel-final interactive prototype** — because this increment proved that wireframe approval (R2) does not predict implementation approval. Art bible §13's territory construction language is the one R1 ruling reopened; the other seven stand. |
| 2026-07-29 | v1.2 — **Phase A exited: R1 and R2 both passed.** Three further owner decisions recorded where they belong: all eight art-bible rulings approved ([03](./03_art_bible.md) §12), **portraits are direction A with the procedural seal as fallback** (VP-02 resolved, [03](./03_art_bible.md) §7.1), and **VP-03 resolves to The Study** ([09](./09_screen_parity_register.md) §4). The workstream decomposition — the artifact §Decisions deferred until the contracts existed — is written: [10_workstream_decomposition.md](./10_workstream_decomposition.md). Phase B begins. |
