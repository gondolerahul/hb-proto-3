...
> The repo is a multi-tenant agentic platform. **Increments 1–5 are built and merged to local `master`**: the one-loop substrate (SIG/GOV/SCH/LOOP+ENV), the sellable Solo Pack MVP (all six Inc-2 workstreams), Pragya v1 + inward auth + realtime voice (Inc 3), the connected business — connectors/SoR on the Zoho flagship plus Pragya's own runtime (Inc 4) — and the Intelligence Engine: model registry, router v1+v2, the EVX admission gate, and GLM/Qwen/Kimi behind the D5 opt-in (Inc 5). The **GenUI Design Gate is passed at design level** (2026-07-24, §3): owner-selected hybrid + the ratified **Vihara** spec ([genui_design_gate_spec.md](./genui_design_gate_spec.md)). **Resume with Increment 6** — its charter is a stub, so start with the clarifying-questions round with Rahul for **LEARN and SEGA** (charter questions 2–3: B10 cross-tenant learning policy, learning-store shape; question 1 is answered by the gate), then write the workstream docs and build **LEARN → SEGA → GENUI** (Vihara's G0 substrate and the §14.5 portrait style boards can start in parallel early; its G5 Glasshouse needs LEARN/EVX). The push in §1 remains the standing external action (this VM has no git write credentials). Keep every gate green; follow the conventions in §5 above.
