...
1. **The push (§1)** — `master` is now **63 commits ahead of `origin/master`** and this VM still has no git write creds. Also **`wip/crm-lead-to-call`** — a preservation snapshot of pre-existing uncommitted work (CRM lead→call + a voice SDK bump) found in the tree at the CONN+SOR session start and branched off post-merge `master` so it wasn't lost; resume or discard it as you see fit.
...
4. **Increment 5 — The Intelligence Engine** (model registry B12 → RTR v1/v2 → fleet + EVX; charter [increment-5/00](./increment-5/00_charter.md)); a clarifying-questions round precedes its docs.
