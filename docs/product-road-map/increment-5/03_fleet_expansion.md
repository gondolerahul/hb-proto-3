...
The fix has three parts, all here: a **conservative default allow-list**, a **per-tenant opt-in with a recorded disclosure**, and a **published data-flow disclosure doc** (subprocessor list, data regions, training-data policy). The fleet grows — GLM (Zhipu), Qwen (Alibaba), Mistral join the catalog — but growth and sovereignty land together, never fleet-first.

## 2. Decision (Rahul, 2026-07-23)

**Conservative default; GLM/Qwen opt-in with disclosure.** The default allow-list is the providers already in production use — **Anthropic, Google, OpenAI/Azure, and Mistral** (Mistral is EU-hosted and does not train on API data, so it clears the conservative bar). **GLM and Qwen are registered but `default_allowed=False`** — a tenant must opt in explicitly, and the opt-in records a disclosure. Chosen over region-gating (a bigger config surface, needs a residency field per tenant we don't have yet) and over the doc-criticized all-on default.
...
GLM (Zhipu), Qwen (Alibaba DashScope), and Mistral all expose **OpenAI-compatible** chat endpoints. So one adapter serves all three:
...
* `ai/llm/router.py::_get_adapter` — **edit**: map `zhipu` / `alibaba` / `mistral` → `OpenAICompatAdapter` with the provider's `base_url` (from `service_metadata`).
* `catalog.py` — append GLM/Qwen/Mistral rows with `capability_profile`, `data_flow` (`default_allowed`: Mistral `True`, GLM/Qwen `False`), an initial `model_prices` window, and `status='preview'` (not `active` — only the EVX gate flips them to `active`, §6).
...
3. **Training-data policy** — explicit per provider: does the vendor train on API traffic (Anthropic/OpenAI-API/Google-Vertex/Mistral: no; GLM/Qwen: stated per their terms and the reason they are opt-in). **And the BabyBuddha statement D5 demands:** the in-house family (Inc 7) post-trains on the platform's *own* agent traces under the export/retention policy — never on another tenant's data, and the fact is disclosed here before any such model can be `active`.
4. **Conservative-default rationale** — why Anthropic/Google/OpenAI/Mistral are default-on and Zhipu/Alibaba are opt-in.
...
| `ai/llm/router.py` | **edit** — `_get_adapter` maps zhipu/alibaba/mistral. |
| `ai/intelligence/catalog.py` | **edit** — GLM/Qwen/Mistral rows (`status='preview'`, `default_allowed` per §4). |
...
2. **T2** — GLM/Qwen/Mistral catalog rows (`preview`) + `capability_profile`/`data_flow`.
...
1. **Conservative default; GLM/Qwen opt-in with disclosure** — Overview §2.4. Mistral clears the conservative bar (EU-hosted, no API-training).
