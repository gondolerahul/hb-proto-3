...
| Date | Change |
|---|---|
| 2026-07-30 | v2.4 — **the Vihara redesign reached full surface coverage.** Branch `inc7/redesign`. R-0 recorded the rejection and locked four decisions; R-1 carried the owner's background across **verbatim** (byte-identity enforced by test, after finding a three-year-old latent bug — `#382b02ff` is invalid for THREE.Color and silently falls back to white, so the intended haze had never once rendered); R-2 built the material system finding RD-5 said was missing, under one rule — *an edge is never one border*; R-3 drew the surfaces. **Owner pick 2026-07-30: the brand re-key**, which put gold in the atmosphere and forced art bible §2.1a — an amendment one layer deep, carrying an explicit obligation to *measure* beacon salience on real hardware rather than assume it. **Owner review round 1** reversed one of my own decisions and was right to: RD-1 was three defects arriving together (skewed **and** colliding **and** too small), and "flat" took the blame for what collision and size did, so ground labels are flat again with the real defects fixed by construction (§2.1b). Two structural additions came out of the review — colleagues personified as **architecture** in `world/Room.tsx`, and art bible direction **A** built procedurally in `components/Portrait.tsx`, which unblocks the halftone busts from the raster pipeline they were waiting on. And review comment D exposed a genuine product gap rather than a styling one: there was **no way for an owner to brainstorm**, so `Brainstorm.tsx` adds the owner-initiated front of the STRAT pipeline — a matter tabled becomes Minutes, then a Proposition, then a Resolution by the same certified act as Pragya's. Next: R-4, which is now a data-source swap rather than a rebuild, because the prototype is real code. |
| 2026-07-30 | v2.3 — **Increment 7 built in full, then the frontend rejected on design.** All eight workstreams merged (SEAM · SUB · WORLD · DRIVER · STEWARD · LINE · GLASS · POLISH) plus three unplanned owner-driven polish rounds — the three rounds being the first signal that the look was not converging. At review the app was rejected on craft: it does not read as though each element was designed with care. **SEAM's backend (`backend/src/ai/genui/`, migration `genui001`) and every D5 contract stand untouched**; the rejection is frontend-only. `vihara/` preserved as `vihara-review-rejected/`. The redesign opens under [increment-7/redesign/00_redesign_charter.md](./increment-7/redesign/00_redesign_charter.md): seven findings diagnosed from the rendered R2 boards, of which **RD-7 is structural** — L9's "every W surface owes a sheet" guarantee gave the nine surfaces a business owner actually works in a *fallback's* design budget. Four decisions locked: IA rebalance promoting those nine to first-class rooms (D1) · the legacy `AnimatedBackground.tsx` ported **verbatim**, brand re-key offered beside it, the gold-budget conflict named not papered (D2) · rejected app as **parts bin** (D3) · review by **pixel-final interactive prototype**, since R2 proved wireframe approval does not predict implementation approval (D4). Standing rule 2 ("docs move with code") is why this is recorded here rather than only in the increment folder. |
